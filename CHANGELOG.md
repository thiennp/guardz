...
### 🔧 Improved
- **`guardWithTolerance`**: Type guard validation runs on a microtask so callers return immediately instead of waiting for the full validation walk

### ⚠️ Changed
- **`guardWithTolerance`**: `callbackOnError` now runs after the function returns (deferred on a microtask). Code that read logged errors synchronously on the next line must await a microtask before asserting on those errors

### 🧪 Testing
- Unit and integration tests flush microtasks when asserting on tolerant validation error callbacks
