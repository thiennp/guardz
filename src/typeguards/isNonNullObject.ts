 ...
 */
export const isNonNullObject: TypeGuardFn<NonNullObject> = function (value, config): value is NonNullObject {
  // Exclude arrays
  const isValid = typeof value === "object" && value !== null && !Array.isArray(value);
  if (!isValid && config) {
    config.callbackOnError(generateTypeGuardError(value, config.identifier, "non-null object"));
  }
  return isValid;
};
