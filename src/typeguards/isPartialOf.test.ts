...
describe('isPartialOf', () => {
  ...
  it('should return true for objects matching the partial shape', () => {
    expect(isPartialTarget({ name: 'Test', age: 30, isActive: true })).toBe(
      true
    );
    expect(
      isPartialTarget({ name: 'Only Name', age: 0, isActive: false })
    ).toBe(true);
    ...
    expect(
      isPartialTargetWithOptional({ name: 'Name only', isActive: true })
    ).toBe(true);
    expect(
      isPartialTargetWithOptional({
        name: 'Name',
        age: 40,
        isActive: false,
      } as any)
    ).toBe(true); // age is ignored
  });
  ...
  it('should return false for non-objects or objects not matching the shape', () => {
    ...
    expect(isPartialTargetWithOptional({ name: 123, isActive: true })).toBe(
      false
    ); // name wrong type
  ...
  it('should handle error reporting for object type and property types', () => {
    ...
    expect(mockCallback).toHaveBeenCalledTimes(1);
    expect(mockCallback).toHaveBeenCalledWith(
      'Expected profile.name (123) to be "string"'
    );
    ...
    expect(mockCallback).toHaveBeenCalledTimes(1);
    expect(mockCallback).toHaveBeenCalledWith(
      'Expected profile.age ("wrong") to be "number"'
    );
